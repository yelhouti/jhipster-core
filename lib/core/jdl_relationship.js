...
class JDLRelationship {
  constructor(args) {
    ...
    this.isInjectedFieldInFromId = merged.isInjectedFieldInFromId;
    this.isInjectedFieldInToId = merged.isInjectedFieldInToId;
  ...
  static checkValidity(relationship) {
    // TODO many to one can't have from as id and vise versa
    const errors = [];
    if (!relationship) {
  ...
  toString() {
    ...
    if (this.injectedFieldInFrom) {
      ...
      string += this.isInjectedFieldInFromId ? ' id' : '';
      string += this.isInjectedFieldInFromRequired ? ' required' : '';
      string += `}`;
    }
    string += ' to';
    if (this.commentInTo) {
    ...
    if (this.injectedFieldInTo) {
      ...
      string += this.isInjectedFieldInToId ? ' id' : '';
      string += this.isInjectedFieldInToRequired ? ' required' : '';
      string += `}`;
    }
...
function defaults() {
  return {
    ...
    isInjectedFieldInFromId: false,
    isInjectedFieldInToId: false,
